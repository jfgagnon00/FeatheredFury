...
@YamlDeserializable
class KerasCNNModelFactory(IFactory):
    """
    Modele base sur les CNN
    """
    def __init__(self):
    ...
    def _create_model(self, 
        ...
        segment_features_shape = (input_shape[1], input_shape[2], 1)

        from ffury.optional.keras_adapters._KerasCNNSegmentFeatures import _KerasCNNSegmentFeatures
        segment_model = _KerasCNNSegmentFeatures(shape=segment_features_shape,
                                                 name="CNNSegmentFeatures")

        group_inputs        = Input(shape=input_shape)
        group_features      = Reshape(target_shape=(input_shape[0], *segment_features_shape))(group_inputs)
        group_features      = TimeDistributed(segment_model,
                                              name="GroupFeatures")(group_features)
        
        # classifier
        group_probabilities = Dropout(rate=0.2)(group_features)
        group_probabilities = Dense(units=64, 
